...
variableCosts: [
  ...
  const handleDeleteItem = (index, setFunction) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const requestData = {
        userId: user.id,
        projectId,
        startupCosts: startupCosts.filter(item => item.description && item.amount),
        startupCapital: startupCapital.filter(item => item.description && item.amount),
        startingOperations: startingOperations.filter(item => item.description && item.amount),
        capitalWorkProgress: capitalWorkProgress.filter(item => item.description && item.amount)
      };
      ...
      // No need to set projectId here as it's already managed by the context
    } catch (err) {
      ...
      <div className="tab-content" id="nav-tabContent">
        <div className={`tab-pane fade ${activeTab === 'nav-startup' ? 'show active' : ''}`} id="nav-startup" role="tabpanel" aria-labelledby="nav-startup-tab">
          <div className="container-fluid form-container">
            ...
            <div className="row">
              <div className="col-6">
                <div className="container-fluid form-container">
                  ...
                  {/* {renderInputs(startupCosts, setStartupCosts)} */}
                  ...
                  {/* {renderInputs(startupCapital, setStartupCapital)} */}
              ...
              <div className="col-6">
                <div className="container form-container">
                  ...
                  {/* {renderInputs(capitalWorkProgress, setCapitalWorkProgress)} */}
                  ...
                  {/* {renderInputs(startingOperations, setStartingOperations)} */}
            ...
            handleClearAll(setStartupCosts);
            handleClearAll(setStartupCapital);
            handleClearAll(setCapitalWorkProgress);
            handleClearAll(setStartingOperations);
          }}>Clear All</button>
...
//   const renderInputs = (items, setFunction) => (
//     items.map((item, index) => (
//       <div className="form-group row" key={index}>
...
//         setError('Session ID not found in response. Please check the server response structure.');
//       }
//     } catch (err) {
//       console.error('Error submitting data:', err);
//       setError(err.response?.data?.error || 'An error occurred while submitting the data. Please try again.');
...
//     } catch (err) {
//       console.error('Error submitting funding data:', err);
//       setError(err.response?.data?.error || 'An error occurred while submitting the funding data. Please try again.');
//     } finally {
//       setIsLoading(false);
//     }
//   };
...
//                     </div>
//                   </div>
//                   {renderInputs(cashFlow, setCashFlow)}
//                 </div>
//               </div>
//               <div className="col-6">
//                 <div className="container form-container">
...
//                     </div>
//                   </div>
//                   {renderInputs(liabilities, setLiabilities)}
...
//                     </div>
//                   </div>
//                   {renderInputs(variableCosts, setVariableCosts)}
//                 </div>
//               </div>
//             </div>
...
// import './entryitems.css';
// import 'bootstrap/dist/css/bootstrap.css';
// import { useSession } from '../../ProjectContext.js'; 
// import { Modal, Button, Form, Table  } from 'react-bootstrap'; 
...
// const years = ['Year1', 'Year2', 'Year3', 'Year4', 'Year5'];
// // New RevenueForecasts component
// const RevenueForecasts = ({ onDataUpdate, initialData = [] }) => {
//   const [showModal, setShowModal] = useState(false);
//   const [monthlyData, setMonthlyData] = useState(
//     initialData && initialData.length === 12
//       ? initialData
//       : months.map(month => ({
//           month,
//           units: '',
...
//   // Use useCallback to memoize the isDataComplete function
...
//         JSON.stringify(initialData) !== JSON.stringify(monthlyData)) {
//       setMonthlyData(initialData);
//     }
//   }, [initialData, monthlyData]);

//   useEffect(() => {
//     // Debounce the onDataUpdate call
...
//     setStartupCosts(testData.startupCosts);
//     setStartupCapital(testData.startupCapital);
//     setCapitalWorkProgress(testData.capitalWorkProgress);
//     setStartingOperations(testData.startingOperations);
//     setFixedExpenses(testData.fixedExpenses);
//     setAssets(testData.assets);
//     setLiabilities(testData.liabilities);
//     setCapitalCosts(testData.capitalCosts);
//     setCashFlow(testData.cashFlow);
//     setRevenueForecasts(testData.revenueForecasts);
//     setVariableCosts(testData.variableCosts);
//     setEmployeePayrolls(testData.employeePayrolls);
//   }, []);


//   const autoSubmitTestData = async () => {
//     setIsLoading(true);
//     setError(null);
//     let currentSessionId = sessionId; // Start with the current sessionId
  
//     try {
//       // populateTestData();
      
//       // Submit startup data and update session ID
//       currentSessionId = await handleSubmit(currentSessionId);
      
//       // Submit funding data and update session ID
//       currentSessionId = await handleFundingSubmit(currentSessionId);
      
//       // Submit operations finance data and update session ID
//       currentSessionId = await handleOperationsFinanceSubmit(currentSessionId);
      
//       // Submit payroll data and update session ID
//       currentSessionId = await handlePayrollSubmit(currentSessionId);
  
//       // Update the session ID state once at the end
//       setSessionId(currentSessionId);
  
//       alert("All test data has been submitted successfully!");
//     } catch (err) {
//       console.error("Error in auto-submitting test data:", err);
//       setError("An error occurred while auto-submitting test data. Please check the console for more details.");
//     } finally {
//       setIsLoading(false);
//     }
//   };
...
//   const handleSubmit = async (currentSessionId) => {
//     setIsLoading(true);
//     setError(null);
...
//   const handleFundingSubmit = async (currentSessionId) => {
//     setIsLoading(true);
//     setError(null);
...
//   const handleOperationsFinanceSubmit = async (currentSessionId) => {
//     setIsLoading(true);
//     setError(null);
...
//       } 
//     } catch (err) {
//       console.error('Error submitting operations data:', err);
//       setError(err.response?.data?.error || 'An error occurred while submitting the data. Please try again.');
//     } finally {
//       setIsLoading(false);
//     }
...
//   // Modify your handlePayrollSubmit function
//   const handlePayrollSubmit = async (currentSessionId) => {
//     setIsLoading(true);
//     setError(null);
...
//     <div className="entryitems d-flex flex-column justify-content-center">
//     <div className="mb-3">
//       <button className="btn btn-secondary me-2" onClick={populateTestData}>
//         Populate Test Data
//       </button>
//       <button className="btn btn-primary" onClick={autoSubmitTestData} disabled={isLoading}>
//         {isLoading ? 'Submitting...' : 'Auto-Submit Test Data'}
//       </button>
//     </div>
//     {/* TEST DATA ABOVE WITH ONE DIV <ATLAST></ATLAST> */}
//     <div className="entryitems d-flex flex-column justify-content-center">
//       <nav className="nav nav-tabs justify-content-center">
//         <div className="nav nav-tabs" id="nav-tab" role="tablist">
//           <button className={`nav-link ${activeTab === 'nav-startup' ? 'active' : ''}`} id="nav-startup-tab" data-bs-toggle="tab" data-bs-target="#nav-startup" type="button" role="tab" aria-controls="nav-startup" aria-selected={activeTab === 'nav-startup'}>Startup Cost</button>
...
//                       <div className="col-sm-3">
//                         <button
//                           className="btn btn-sm me-2 btnEditDelete"
//                           onClick={() => handleModalOpen('fixedExpense', index)}
//                         >
//                           <AiTwotoneEdit />
//                         </button>
...
//                       </div>
//                       <div className="col-sm-3">
//                         <button className="btn btn-sm btnEditDelete me-2" onClick={() => handleModalOpen('capitalCost', index)}><AiTwotoneEdit /></button>
//                         <button className="btn btn-sm btnEditDelete" onClick={() => handleDeleteCapitalCost(index)}><AiFillDelete /></button>
//                       </div>
//                     </div>
//                   ))}
//                 </div>
...
//             <div className="row">
//               <div className="col-6">
//               <RevenueForecasts onDataUpdate={handleRevenueDataUpdate} initialData={revenueForecasts} />
//               </div>
//               <div className="col-6">
//                 <div className="container-fluid form-container">
//                   <h6>Variable Costs:</h6>
